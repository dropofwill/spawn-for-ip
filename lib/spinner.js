...
exports.createSpinner = function(globalOptions) {
	...
	globalOptions.idleTimeSec = globalOptions.idleTimeSec || (30 * 60); // default 30 minutes before idling a process
	...
	function createFsm(options) {
		...
		var spinner = {
			...
			stopped: {
				...
				changed: function(cb) {
					spinner.logger.log('file changed while stopped. nothing to do');
					return cb();
				},
			...
			findport: {
							...
							spinner.child.stderr.on('data', function(data) { 
								...
								if (data) {
									...
									c.split('\n').forEach(function(line) {
										spinner.logger.error('[stderr]', line);
									});
								}
				...
				stop: function(cb) {
					spinner.logger.log('stop triggered while starting');
				...
				changed: function(cb) {
					spinner.logger.log('file changed, ignoring because it was still not started');
					return cb();
				},
			},
			...
			wait: {
				...
				stop: function(cb) {
					spinner.logger.log('stop waiting for child to bind');
				...
				changed: function(cb) {
					spinner.logger.log('file changed while waiting for process to bind, should restart');
			...
			faulted: {
				$enter: function(cb) {
					spinner.logger.warn('child process terminated. waiting for a change to restart');
					qemitError(new Error("unable to start child process"));
					return cb();
				},

				start: function(cb) {
					qemitError(new Error('child in faulted state, can only start by modifying the file'));
					return cb();
				},
				...
				changed: function(cb) {
					spinner.logger.log('file changed while in faulted, restart');
			...
			started: {
				...
				changed: function(cb) {
					spinner.logger.log('"' + spinner.options.monitor + '"', 'changed. restarting');
					spinner.state = 'restarting';
					return cb();
				},
			},
			...
			restarting: {
				...
				start: function(cb) {
					spinner.logger.log('start while restarting is noop');
					return cb();
				},

				stop: function(cb) {
					spinner.logger.log('stop while restarting');
					spinner.state = 'stopping';
					//TODO: test
					return cb();
				},
				...
				changed: function(cb) {
					spinner.logger.log('changed while restarting is noop');
					return cb();
				}
			},
			...
			stopping: {
				$enter: function(cb) {
					...
					if (spinner.child) {
						spinner.logger.log('killing child', spinner.child.pid);
						spinner.child.kill('SIGKILL');
					}
					else {
						// the child is already killed, simulate a 'term'
						spinner.logger.log('child already terminated, just moving ahead');
						spinner.trigger('term');
					}
				...
				start: function(cb) {
					spinner.logger.log('start while stopping.. ignore in the meantime i guess');
				...
				changed: function(cb) {
					spinner.logger.log('file changed while stopping. since we are stopping anyway, we dont care');
					return cb();
				},
			},
		};
		...
		function restartIdleTimer() {
			if (spinner.options.idleTimeSec) {
				if (spinner.idleTimer) {
					spinner.logger.log('clearning previous idle timer');
					clearTimeout(spinner.idleTimer);
				}

				spinner.logger.log('starting idle timer for', spinner.options.idleTimeSec, 'seconds');
		...
		function clearIdleTimer() {
			if (spinner.idleTimer) {
				spinner.logger.log('clearing idle timer');
				clearTimeout(spinner.idleTimer);
			}
		}
