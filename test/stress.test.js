function createTests(portalloc) {
	...
	tests.monitor = function(test) {
		...
		copy(version1, target, function(err) {
			...
			var child = self.spinner.start(target, function(err, port1) {
				...
				return request(port1, function(err, res, body) {
					...
					child.on('restarted', function(port2) {
						...
						request(port2, function(err, res, body) {
	...
	function request(port, callback) {
		...
		return http.get(req, function(res) {
			var buff = '';
			res.on('data', function(data) { buff += data.toString(); });
			res.once('end', function() {
				return callback(null, res, buff);
			});
	...
	return require('nodeunit').testCase(tests);
}

var staticports = require('../lib/staticports');
var dynamicports = require('../lib/dynamicports');
exports.staticports = createTests(staticports());
exports.dynamicports = createTests(dynamicports());